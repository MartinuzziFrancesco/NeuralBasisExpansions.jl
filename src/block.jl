...
struct GenericBasis <: AbstractBasis
    backcast_size::Int
    forecast_size::Int
end
...
struct TrendBasis <: AbstractBasis
    polynomial_size::Int
...
function TrendBasis(degree_of_polynomial::Int, backcast_size::Int, forecast_size::Int)
    ...
    TrendBasis(polynomial_size, backcast_time, forecast_time)
end

function (basis::TrendBasis)(theta::AbstractArray)
    backcast = theta[:, basis.polynomial_size+1:end] * basis.backcast_time
    forecast = theta[:, 1:basis.polynomial_size] * basis.forecast_time
    return backcast, forecast
end

struct SeasonalityBasis <: AbstractBasis
    backcast_cos_template::Array{Float32, 2}
    backcast_sin_template::Array{Float32, 2}
    forecast_cos_template::Array{Float32, 2}
    forecast_sin_template::Array{Float32, 2}
end
...
function SeasonalityBasis(harmonics::Int, backcast_size::Int, forecast_size::Int)
    ...
    SeasonalityBasis(backcast_cos_template, backcast_sin_template, forecast_cos_template, forecast_sin_template)
end

function (basis::SeasonalityBasis)(theta::AbstractArray)
    params_per_harmonic = size(theta, 2) ÷ 4
    backcast_harmonics_cos = theta[:, 2 * params_per_harmonic + 1:3 * params_per_harmonic] * basis.backcast_cos_template
    backcast_harmonics_sin = theta[:, 3 * params_per_harmonic + 1:end] * basis.backcast_sin_template
    backcast = backcast_harmonics_sin + backcast_harmonics_cos

    forecast_harmonics_cos = theta[:, 1:params_per_harmonic] * basis.forecast_cos_template
    forecast_harmonics_sin = theta[:, params_per_harmonic + 1:2 * params_per_harmonic] * basis.forecast_sin_template
